...
# Prompt Template
# =============================================================================
def create_prompt_template_for_revision_step_1(
...
def create_prompt_template_for_revision_step_2(
    prompt_manager: PromptManager,
    prompt_version: str,
    input_variable: dict,
    revision_data: dict[str, List]
) -> (ChatPromptTemplate, dict):
    ...
    input_variable["json_format"] = prompt_contents["json_format"]
    input_variable["revision_data"] = revision_data
...
# Chain & invoke
# =============================================================================
async def generate_revision_data(
    ...
    for idx in range(len(blocks)):
        ...
        if check_only_whitespace_or_newline(blocks[idx]["text"]):
            blocks_result[-1]["revised_text"] = blocks[idx]["text"]
            continue
    ...
    revision_data = {
        "blocks_result": llm_results
    }

    return revision_data


async def generate_tagged_text_data(
    prompt: ChatPromptTemplate,
    model,
    input_variable: dict,
):
    ...
    blocks = input_variable["blocks"]
    revision_data = input_variable["revision_data"]
    json_format = input_variable["json_format"]
...
def compare_original_and_revised_text(
    ...
    for idx, revision in enumerate(revision_list):
        pattern = f'<fix>{re.escape(revision["revised_text"])}</fix>'
